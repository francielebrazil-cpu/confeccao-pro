...
export default function App() {
  ...
  const supabaseInsert = async <T = any>(table: string, values: object | object[]) => {

    const { data, error } = await supabase.from<T>(table).insert(values).select('*');
  ...
  const supabaseUpdate = async <T = any>(table: string, values: object, id: number) => {

    const { data, error } = await supabase.from<T>(table).update(values).eq('id', id).select('*');
  ...
  const supabaseDelete = async (table: string, id: number) => {
  ...
  const supabaseDeleteMany = async (table: string, ids: number[]) => {
    ...
    try {
      ...
      if (isSupabaseConfigured) {
        ...
        if (data) {
          ...
          setFormDocType((data as CompanySettings).document_type || 'CNPJ');
